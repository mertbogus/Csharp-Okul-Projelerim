                ...
                if (tus == DialogResult.Yes)
                {
                    for (int i = 0; i < dataGridView2.Rows.Count; i++)
                    {
                        if (Convert.ToBoolean(dataGridView2.Rows[i].Cells[0].Value) == true)
                        {
                            ...
                            int ks1 =sorgu.ExecuteNonQuery();
                            
                        }
                    }
                    if (ks1 > 0)
                    {
                        MessageBox.Show("Seçili Olan Dersler, İlgili Öğrenci İçin Kayıt Edilmiştir.", "Ders Kayıt İşlemi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception Ex)
