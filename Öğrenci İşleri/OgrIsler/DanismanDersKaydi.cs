...
namespace OgrIsler
{
    public partial class DanismanDersKaydi : Form
    {
        public DanismanDersKaydi()
        {
            InitializeComponent();
        }
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                sorgu.CommandText = "select * from ogr_danisman where bkodu=@bkodu";
                sorgu.Parameters.AddWithValue("@bkodu", comboBox1.SelectedValue);
                SqlDataAdapter adp = new SqlDataAdapter(sorgu);
                DataTable dt = new DataTable();
                adp.Fill(dt);
                comboBox2.DisplayMember = "dadi";
                comboBox2.ValueMember = "dkodu";
        ...
        private void DanismanDersKaydi_Load(object sender, EventArgs e)
        {
            bolumlistele(); 
        }
                    ...
                    for (int i = 0; i < dataGridView2.Rows.Count; i++)
                    {
                        if (Convert.ToBoolean(dataGridView2.Rows[i].Cells[0].Value) == true)
                        {
                            ...
                            sorgu.Parameters.AddWithValue("@derskodu", dataGridView2.Rows[i].Cells[1].Value);
                             ks1=sorgu.ExecuteNonQuery();
                            
                        }
                    }
                    if (ks1 > 0)
                    {
                        MessageBox.Show("Ders Kaydı Gerçekleştirilmiştir.", "Bilgi Ekranı", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception Ex)
